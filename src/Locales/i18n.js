...
const defaultTimezoneName = defaultTimezone.name();
...
(function(factory) {
  ...
  I18n.locales['default'] = function(locale) {
    ...
    // listen to fallbacks
    if (typeof I18n.fallbacks === 'object') {
      // giving specific fallback list
      fallbackList = I18n.fallbacks[locales[0]];
      if (!fallbackList)
        fallbackList = [];
      if (typeof fallbackList === 'string')
        fallbackList = [fallbackList];

      locales = locales.concat(fallbackList);
    } else if (I18n.fallbacks && I18n.defaultLocale) {
  ...
  I18n.parseDate = function(date) {
    ...
    if (matches) {
      ...
      convertedDate = new _Date();
      convertedDate.setTime(_Date.parse(date));
    } else {
